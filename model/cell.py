...
class SSU(Function):  # Smooth Sparse Units "SSU":"ssu"
    ...
    def backward(ctx, grad_output):
        ...
        grad_input[(t < sup) & (t > 0)] = grad_output[(t < sup) & (t > 0)]*2
...
class FAGRUCell(torch.nn.Module):
    ...
    def forward(self, inputs, hx, adj):
        ...
        # print("value_1.shape:{}".format(value.shape))
        value = torch.reshape(value, (-1, self._num_nodes, output_size))
        # print("value_2.shape:{}".format(value.shape))
        r, u = torch.split(tensor=value, split_size_or_sections=self._num_units, dim=-1)
        # print("r.shape:{}  u.shape:{}".format(r.shape, u.shape))
